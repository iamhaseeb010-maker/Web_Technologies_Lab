...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (file_exists("users.txt")) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($data) >= 3 && $data[1] == $email && $data[2] == $password) {
...
<head>
    ...
    <title>Abdul Haseeb | Secure Login</title>
...
<body class="mesh-gradient min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-emerald-600 font-medium">Lab 3 | Access your secure dashboard</p>
        </div>

        <div class="bg-white/80 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-2xl shadow-emerald-900/10 border border-white">
            
            <?php if ($success): ?>
                <div class="mb-6 p-4 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-100 flex items-center gap-3 animate-pulse">
            ...
            <form method="POST" action="login.php" class="space-y-6">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center ml-1">
                        ...
                        <a href="#" class="text-xs font-semibold text-emerald-600 hover:text-emerald-700 transition">Forgot?</a>
                    </div>
            ...
            <div class="mt-10 pt-6 border-t border-emerald-50 text-center">
                <p class="text-emerald-900/50 text-sm font-medium">
                    New to the system? 
                    <a href="signup.php" class="text-emerald-600 font-bold hover:text-emerald-700 transition-colors ml-1">Create Account</a>
                </p>
            </div>
        </div>
        
        <p class="text-center mt-8 text-emerald-900/30 text-xs font-bold uppercase tracking-widest">
            Abdul Haseeb &bull; Secure Authentication
        </p>
    </div>
